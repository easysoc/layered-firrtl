...
import layered.elknodes._
import layered.stage.cli.{FlattenLevelAnnotation, StartModuleNameAnnotation}
import layered.util.Scope

import java.io.PrintWriter
import scala.collection.mutable

class MakeOne extends Transform with DependencyAPIMigration {
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    val maxDepth = state.annotations.collectFirst {
      case FlattenLevelAnnotation(depth) => depth
    }.getOrElse(1)
